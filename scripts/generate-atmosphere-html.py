#!/usr/bin/env python3
"""从 /tmp/atmosphere-sections.json 生成谈话氛围控制 HTML 专题。
输出: docs/谈话氛围控制-从播客中学到的对话调频.html
"""
import json
import html

data = json.load(open('/tmp/atmosphere-sections.json', encoding='utf-8'))
...
def build_map():
    ...
    return f"""    <div class="map">
      <h2>氛围总览：会聊天的人，都在偷偷调频</h2>
      <p class="map-intro">从 {PODCASTS} 档中文播客、120 篇完整转录中，提炼出 {TOTAL_CARDS} 条真实氛围控制动作。每一类都配了真实原句、对话场景和日常复用方法。建议按板块对照练习：先学会化解尴尬，再学会让对话发光。</p>
      <div class="legend">
        {'\n        '.join(legend)}
      </div>
      <div class="nav">
...
<title>谈话氛围控制 · 从播客中学到的对话调频</title>
<style>
{css}
</style>
</head>
...
<div class="html-toolbar">
  ...
  <div class="html-toolbar-title">谈话氛围控制 · 从播客中学到的对话调频</div>
</div>
<div class="container">
    <h1>谈话氛围控制 · 从播客中学到的对话调频</h1>
    <p class="subtitle">从 120 篇完整转录中提炼 · 158 条候选 → 精选 {TOTAL_CARDS} 条 · 覆盖 {PODCASTS} 档播客</p>

{map_html}
{sections_html}
    <div class="conclusion">
      <h2>一句话带走</h2>
      <p class="conclusion-text">气氛不是天赋，是动作：尴尬时自嘲救场，陌生时示弱拉近，紧绷时幽默缓冲，敏感时安全铺垫，难过时共情倾听，跑偏时节奏转场，让每个人都被托举——七招调频，就是那些"会聊天"的人的底层操作。</p>
    </div>

    <div class="footer">
      <p>音频工作坊 · 谈话氛围控制 · 2026 · 覆盖 {PODCASTS} 档播客</p>
    </div>
  </div>
</body>
</html>
"""
    out_path = 'docs/谈话氛围控制-从播客中学到的对话调频.html'
